...
type maxHeap []engineResult

func (h maxHeap) Len() int           { return len(h) }
func (h maxHeap) Less(i, j int) bool { return h[i].score > h[j].score } // Omgekeerd voor max-heap
func (h maxHeap) Swap(i, j int)      { h[i], h[j] = h[j], h[i] }
func (h *maxHeap) Push(x interface{}) { *h = append(*h, x.(engineResult)) }
...
func evaluateBatch(engines []string, inputEngines []string, top100000Chan chan<- engineResult, progressComparisons *int64, numInputEngines int) {
    ...
    for i := 0; i < len(engines); i += subBatchSize {
        ...
        // Verwerk de resultaten (bijv. top 100.000 slechtste scores)
		h := &maxHeap{}
		heap.Init(h)
		maxSize := 100000
...
func main() {
	for {
		...
		if top100000.Len() > 0 {
			...
			// Sorteer de resultaten in oplopende volgorde van score (slechtst naar beter)
			...
			fmt.Printf("Top 100,000 slechtste engines opgeslagen uit %d matches.\n", totalMatches)
		} else {
